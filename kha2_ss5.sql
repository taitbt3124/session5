...
select product_name, sum(total_price) as total_revenue from kha1.orders
  inner join kha1.products p on p.product_id = orders.product_id
  group by p.product_name
  having sum(total_price) =
      (select max(total_price) "Tổng cao nhất" from kha1.orders);
